...
KModule,getPriv = KClass(function(moduleName,entryPoint,env)
    KError.ValidateArg("moduleName",KVarConditions.String(moduleName))
    KError.ValidateArg("entryPoint",KVarConditions.Function(entryPoint))
    if env then KError.ValidateArg("env",KVarConditions.Table(env)) end
    ...
    do --general
        ...
        function addDisposeCB(key,callback)
            if callback then KError.ValidateArg("callback",KVarConditions.Function(callback)) end
