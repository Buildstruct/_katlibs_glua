...
KModelData,getPriv = KClass(function(pos,ang,model)
    KError.ValidateArg("pos",KVarConditions.Vector(pos))
    KError.ValidateArg("ang",KVarConditions.Angle(ang))
    KError.ValidateArg("model",KVarConditions.String(model))
...
function KModelData:SetColor(value)
    ...
    KError.ValidateArg("value",KVarConditions.Color(value))
...
function KModelData:SetRenderGroup(value)
    ...
    KError.ValidateArg("value",KVarConditions.NumberInRange(value,7,9))
...
function KModelData:SetRenderMode(value)
    ...
    KError.ValidateArg("value",KVarConditions.NumberInRange(value,0,10))
...
function KModelData:SetClip(key,localPos,localNormal)
    ...
    if localPos == nil and localNormal == nil then
        ...
        KError.ValidateArg("localPos",KVarConditions.Vector(localPos))
        KError.ValidateArg("localNormal",KVarConditions.Vector(localNormal))
...
function KModelData:SetSubmaterial(index,value)
    KError.ValidateArg("index",KVarConditions.NumberGreater(index,0))
    KError.ValidateArg("value",KVarConditions.String(value))
